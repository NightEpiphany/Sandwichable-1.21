...
public class CustomMinecartItem extends InfoTooltipItem {
    private static final DispenserBehavior DISPENSER_BEHAVIOR = new ItemDispenserBehavior() {
        ...
        public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            double x = pointer.centerPos().getX() + (double)dir.getOffsetX() * 1.125D;
            double y = Math.floor(pointer.centerPos().getY()) + (double)dir.getOffsetY();
            double z = pointer.centerPos().getZ() + (double)dir.getOffsetZ() * 1.125D;
