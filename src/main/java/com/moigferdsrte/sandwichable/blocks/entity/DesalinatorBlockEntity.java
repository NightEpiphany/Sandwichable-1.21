...
public class DesalinatorBlockEntity extends LockableContainerBlockEntity implements ExtendedScreenHandlerFactory<DesalinatorPos>, SidedInventory, SyncedBlockEntity {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, DesalinatorBlockEntity self) {
        if(world.getBlockState(pos).getBlock() == BlocksRegistry.DESALINATOR) {
            ...
            FluidType fluid = world.getBlockState(pos).get(DesalinatorBlock.FLUID);
            if(fluid != FluidType.NONE && self.fluidAmount < maxFluidAmount && (((fluid == FluidType.PICKLE_BRINE) == self.isPickleBrine) || self.fluidAmount == 0)) {
                self.isPickleBrine = fluid == FluidType.PICKLE_BRINE;
                world.setBlockState(pos, world.getBlockState(pos).with(DesalinatorBlock.FLUID, FluidType.NONE));
                self.fluidAmount++;
    ...
    @Override
    public DesalinatorPos getScreenOpeningData(ServerPlayerEntity player) {
        return new DesalinatorPos(this.pos);
    }
}
