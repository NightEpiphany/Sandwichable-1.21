...
public class BasinBlockEntity extends BlockEntity implements SidedInventory, SyncedBlockEntity {
    ...
    public ItemStack insertMilk(ItemStack milk) {
        ...
        if(milk.getItem() == Items.MILK_BUCKET) {
            ...
            if(milk.contains(CUSTOM_DATA) && milk.get(CUSTOM_DATA).copyNbt().getCompound("bucketData") != null) {
                NbtCompound tag = milk.get(CUSTOM_DATA).copyNbt().getCompound("bucketData");
    ...
    public ItemStack extractMilk() {
        ...
        if(content == BasinContent.MILK) {
            ...
            if (stack.contains(CUSTOM_DATA)) NbtComponent.set(CUSTOM_DATA, stack, nbt -> nbt.put("bucketData", tag));
            else {
                NbtCompound nbt = new NbtCompound();
                nbt.put("bucketData", tag);
                NbtComponent.set(CUSTOM_DATA, stack, nbt);
            }
            emptyBasin();
            update();
            return stack;
    ...
    public void insertCheese(ItemStack cheese) {
        if(cheese.getItem() instanceof CheeseItem && !((CheeseItem)cheese.getItem()).isSlice()) {
            content = CheeseRegistry.INSTANCE.cheeseFromCheeseType(((CheeseItem)cheese.getItem()).getType());
            cheese.decrement(1);
        }
        update();
    }
