...
public class ToasterBlockEntity extends BlockEntity implements SidedInventory, SyncedBlockEntity {
    ...
    private void toastItems() {
        for (int i = 0; i < 2; i++) {
            SingleStackRecipeInput inv = new SingleStackRecipeInput(items.get(i));
            ...
            if(match.isPresent()) {
                items.set(i, match.get().value().craft(inv, world.getRegistryManager()).copy());
                changed = true;
            } else {
                if(items.get(i).contains(FOOD)) {
